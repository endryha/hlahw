...
print "mysql_master: log=$CURRENT_LOG, position=$CURRENT_POS"
...
show_slave_status_cmd="export MYSQL_PWD=111; mysql -u root -e 'SHOW SLAVE STATUS \G'"

docker exec mysql_slave_1 sh -c "$start_slave_cmd"
docker exec mysql_slave_1 sh -c "$show_slave_status_cmd"

docker exec mysql_slave_2 sh -c "$start_slave_cmd"
docker exec mysql_slave_2 sh -c "$show_slave_status_cmd"
